...
CPoint Game::normalizeÑoordinates(CPoint pos)
{
	CPoint norm_pos(pos);
	while (norm_pos.x < 0){
...
bool Game::doActionMove(UINT robot_id, INT dx, INT dy)
{
	...
	CPoint new_pos = normalizeÑoordinates(CPoint(self_robot->x + dx, self_robot->y + dy));
	
	//Íåïîíÿòíàÿ ñèòóàöèÿ â ñëó÷àå ïåðåìåùåíèÿ ðîáîòîâ â îäíó êëåòêó
...
DWORD WINAPI robotThread(LPVOID lpParam)
{
	...
	__try{
		if (threadStruct->robotFunction != NULL){
			...
			AfxMessageBox("Ôóíêöèÿ íå èíèöèàëèçèðîâàíà");
		}
	}
	__except(EXCEPTION_DEBUG_EVENT){
		AfxMessageBox("Îøèáêà ôóíêöèè dll");
		//exit(-1);
		return -1;
	}
	return 1;
...
bool Game::setRobotsInfo(CString filePath)
{
	...
	while (f.getline(buffer, 1024)) {
		...
		if (!pch){
			AfxMessageBox("Îøèáêà ÷òåíèÿ ïóòè ê dll");
			return 0;
		}
		CString path(pch);
		...
		pch = strtok(NULL, " ");
		if (!pch){
			AfxMessageBox("Îøèáêà ÷òåíèÿ èìåíè ðîáîòà");
			return 0;
		}
		CString name(pch);
		...
		pch = strtok(NULL, " ");
		if (!pch){
			AfxMessageBox("Îøèáêà ÷òåíèÿ èìåíè àâòîðà");
			return 0;
		}
		string author(pch);
	...
	for (auto it = m_Robots_global.begin(); it != m_Robots_global.end(); ++it){
		if ((*it)->isPlayer){
			...
			if ((*it)->hDll == NULL){
				CString str;
				str.Format("Îøèáêà çàãðóçêè dll: %s", (*it)->filePath);
				AfxMessageBox(str);
				return 0;
			}
			...
			if ((*it)->robotFunction == NULL){
				CString str;
				str.Format("Ôóíêöèÿ DoStep íå íàéäåíà â %s", (*it)->filePath);
				AfxMessageBox(str);
				return 0;
			}
		}
	...
	if (standartDll == NULL){
		CString str;
		str.Format("Îøèáêà çàãðóçêè dll ñòàíäàðòíîãî ðîáîòà: %s", STANDART_ROBOT_DLL);
		AfxMessageBox(str);
		return 0;
	}
...
//				if (distance < 2 * m_Config.V_max) {
			...
			++num;
			if (num > 100000){
				AfxMessageBox("Íå ìîãó ðàññòàâèòü ðîáîòîâ :'(");
				m_isNormalEnd = true;
				end();
				return 0;
			}
	...
	while (m_StepNum < m_Config.N && nAliveRobots > 1) {
		...
		//Ôîðìèðîâàíèå ñòðóêòóðû StepInfo
		StepInfo stepInfo;
		for (auto it = m_Robots_start.begin(); it != m_Robots_start.end(); ++it){
...
void Game::endRound()
{
	...
	//Îáðàáîòêà ðåçóëüòàòîâ ðàóíäà
	for (USHORT i = 0; i < size; ++i){
		Robot* cur_robot = m_Robots_ptr[i];
		m_RoundResults[i] = cur_robot;

		//Íà÷èñëåíèå î÷êîâ çà óáèéñòâà â ðàóíäå
		if (cur_robot->Alive){
...
BOOL DeleteFolder(LPCSTR szPath)
{
	...
	//Åñëè îáüåêò íå äèðåêòîðèÿ 
	if (~dwAttrs & FILE_ATTRIBUTE_DIRECTORY)
		return FALSE;

	SetLastError(0);

	//Ïðîáóåì ñðàçó óäàëèòü äèðåêòîðèþ
